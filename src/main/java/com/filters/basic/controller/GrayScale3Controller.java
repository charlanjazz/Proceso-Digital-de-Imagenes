 ...
 * @author chars
 */
@ManagedBean(name="gs3")
@ViewScoped
public class GrayScale3Controller implements Serializable {
    ...
    public void aplicar() {
        editor.setImg(grayScale3(editor.getImg2()));
    }
    
    public BufferedImage grayScale3(BufferedImage img){
        ...
        for(int i = 0; i<w; i++){
            for(int j = 0; j<h; j++){
                ...
                int gs = (int)((c.getRed() * 0.2126) + (c.getGreen() * 0.7152) + (c.getBlue() * 0.0772));
